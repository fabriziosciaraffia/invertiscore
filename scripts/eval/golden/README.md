...
| `info-indicadores-catch-test.ts` | El ⓘ de los indicadores (23-sep-2026, mockup `info-indicadores.html`, opción A): la pila de hojas (`hoja-pila.ts`) ejercitada con historial y teclado falsos —atrás y Esc cierran solo la de arriba, la ✕ consume su entrada sin tocar la de abajo, el popover de escritorio entra a la pila sin historial— y su cableado (Modal variante `glosa` en portal, el segundo nivel sin bloquear el body, z-index); «retorno sobre lo puesto» no se dibuja (fuera, declarados, los prompts de IA); el cap rate STR sin «neto» ni «por día»; el hero y el capítulo I STR con la misma referencia (`referenciaCapRateStr`, renderizado con zona 2,2 y sin zona); sin cursiva ni guiones en cap rate y cash on cash; FilaDato renderizado sin `title`; los cortes del score a constante y citados por el ⓘ del Franco Score. Verificado en rojo con 24 mutaciones. | 0 |
| `capas-oscuro-catch-test.ts` | Las capas en oscuro (23-sep-2026, aprobado sobre `shots-capas/`): con los tokens oscuros de la portada y los velos compuestos, cada capa (página → hoja grande → hoja chica → popover) tiene ΔL* ≥ 4 sobre la de abajo; todo el bloque va detrás de `html:not([data-theme="light"])`; la subida de la escalera se captura en la hoja (`--esc-*`) y los hijos y el `.doc-tokens` la redeclaran sin leerse a sí mismos. Lo del DOM lo mide `capas-oscuro-sonda.ts` (standalone, `--base`): ninguna pieza del color de su contenedor en 390 y 1100, la escalera en el DOM y el claro sin cambio. Verificado en rojo con 8 mutaciones. | 0 |
| `sin-semaforo-catch-test.ts` | Sin ocre ni verde fuera de la tríada (23-sep-2026, decisión de Fabrizio): ningún archivo de `src` consume `var(--doc-good)` / `var(--doc-warn)`, sus hex (#2E8B57 · #57B98A · #B7791F · #DFA34F) solo en la línea que declara el token, y las zonas del Dial leen la tríada con nombre (`--verdict-buscar/-ajusta/-comprar`, mismos hex que `[data-verdict]`). Lo del DOM lo mide `sin-ocre-verde-sonda.ts` (standalone, `--base`): nada visible ocre ni verde en 7 filas, las dos modalidades, 390, oscuro y claro, con cada hoja abierta; exceptúa `[data-mapa-leyenda]` (paleta de categorías del mapa, espera decisión). Verificado en rojo con 7 mutaciones. | 0 |
| `planilla-calculo-catch-test.ts` | «Cómo se calcula» del mockup aprobado (23-sep-2026, `planilla-como-se-calcula.html`): la venta no existe en ninguna de las dos planillas (queda la fila que remite a «Tu resultado», renderizada con su enlace); el cash on cash lee el campo del motor —sobre las seeds, con y sin corretaje, flujo anual ÷ `capitalCashOnCash` da `cashOnCash`, y la cuenta LTR divide por ese campo y la STR por `capitalInvertido`—; vacancia, entrega y comisión salen del dato (funciones puras ejercitadas y cableado desde el input, sin «0,6 mes», «Entrega inmediata» ni «comisión de plataforma 3%» a mano); la moneda sigue el toggle (render en UF sin «$», pesos exactos en escritorio y millones con el aviso en teléfono, los dos padres pasan `currency` y el enlace abre «Tu resultado»); el pie STR con `referenciaCapRateStr`; el CSS viejo fuera. Lo del DOM lo mide `planilla-calculo-sonda.ts` (standalone, `--base`): tres filas × 390/350/1100 × claro/oscuro × CLP/UF. Verificado en rojo con 16 mutaciones (y la sonda con una). | 0 |
| `cap-rate-neto-catch-test.ts` | Una sola «cap rate neto» LTR (23-sep-2026, decisión de Fabrizio tras el censo: hero y pop-up diferían en 1.212 de 1.213 filas): es `rentabilidadNeta` —el NOI de mercado— y `capRate` no se muestra. El hero renderizado con un `capRate` centinela no lo imprime; el anexo tampoco; en `src` nadie lee `metrics.capRate` (salvo la normalización de filas viejas) ni rotula «Rent. Operativa». Hero, pop-up (sonda sin parche) y hallazgo dan la misma cifra en las seeds; la planilla y el prompt citan la neta. El capítulo I no tiene rama neta: todos los peldaños en base bruta, el nacional 4,0 ÷ 0,8 = 5,0 bruto. Verificado en rojo con 11 mutaciones. | 0 |
| `finding-display-legacy-catch-test.ts` | La card de un hallazgo GUARDADO no revienta (23-sep-2026): `findingDisplay` leyó `valor.sujetoPct` sin mirar y los hallazgos anteriores al 21-sep no lo traen — rompió el build de master (TypeError en el prerender de `/dev/finding-card`) y el anexo y el PDF STR con filas viejas. Fija: el `cap_rate` en formato viejo se muestra con su `capRatePct`, sin «bruto»; el actual dice el bruto; los hallazgos guardados de los fixtures del repo (358) pasan sin tirar ni decir «undefined»; el capítulo I cae a `capRatePct`; la página dev conserva el formato viejo para que `next build` lo ejercite. Verificado en rojo con la versión rota de 64b6e1ca. | 0 |
| `factibilidad-demanda-catch-test.ts` | La factibilidad STR mide la demanda de la zona (23-sep-2026, `ACTAS-factibilidad-demanda.md`): salen la constante de regulación, el ingreso del estimador y la tipología —ni en el código ni en las entradas, y la dimensión entera es la curva (mediana → 65, mitad → 25, 1,6× → 90)—; la ocupación realizada entra por su NIVEL y no por la brecha (no se mueve con la ocupación estimada); sin comparables, atractores; el recálculo toma el dato de los results y las 8 entradas de servidor se los pasan por `conOcupacionRealizadaDelCache`, que lee con el cliente de servicio y no escribe; y la huella del filtro de listings es la que guarda el ancla (`generar-ocupacion-realizada-santiago.ts`). Lo que necesita base lo mide `factibilidad-demanda-sonda.ts` (standalone): las 42 filas sin el dato lo recuperan del caché. Verificado en rojo con 9 mutaciones (y la sonda con el cliente anónimo). | 0 |
