...
// (recomendacion + banda + fragil + porAbsoluto + drivers) en ambas-baseline.json. El flip de
// gestión salió el 25-sep-2026: ver el acta en ambas-recompute.ts.
...
const payload = {
  note: "Baseline GOLDEN COMPARATIVO (AMBAS · D1+D2). Congela veredicto+banda+señales de 7 pares reales, " +
    ...
    "toca), LTR_PREFERIDO, INDIFERENTE(5-15). recomendacion/banda/fragil/porAbsoluto son DUROS; " +
...
for (const [k, b] of Object.entries(seeds)) {
  console.log(`  ${k}: ${b.banda} reco=${b.recomendacion} fragil=${b.fragil} · pirámide=[${b.findingsOrden}]`);
}
