# scrape-toctoc-venta.ps1 - Scraper TocToc SOLO VENTAS (usado + nuevo)
# Correr: .\scrape-toctoc-venta.ps1
# Delay de 2s entre comunas para evitar 403
...
$TIPO_VENTA = @{ slug = "venta"; filtroId = 1; filtroValue = @(1, 2) }

function Get-TocTocSession {
    Write-Host "Obteniendo sesion de TocToc..." -ForegroundColor Cyan
    $null = Invoke-WebRequest -Uri "https://www.toctoc.com/venta/departamento/metropolitana/santiago" -Headers @{"User-Agent"="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"} -UseBasicParsing -SessionVariable s
...
Write-Host ""
Write-Host "FRANCO - Scraper TocToc VENTAS (24 comunas, max $MAX_PAGES pags/comuna)" -ForegroundColor Red
...
Write-Host ""
Write-Host "=== VENTA (usado + nuevo) ===" -ForegroundColor Yellow

foreach ($comunaSlug in $COMUNAS.Keys | Sort-Object) {
    $comunaInfo = $COMUNAS[$comunaSlug]
    $props = Get-Properties -Session $session -ComunaSlug $comunaSlug -ComunaInfo $comunaInfo -Tipo $TIPO_VENTA
    $allProperties += $props
    ...
    # Delay entre comunas para evitar 403
    Start-Sleep -Seconds 2
}

Write-Host ""
$ventaUsado = ($allProperties | Where-Object { $_.condicion -eq "usado" }).Count
$ventaNuevo = ($allProperties | Where-Object { $_.condicion -eq "nuevo" }).Count
Write-Host "Total scrapeado: $($allProperties.Count) ventas" -ForegroundColor Cyan
Write-Host "  Venta usado: $ventaUsado | Venta nuevo: $ventaNuevo" -ForegroundColor Gray
...
Write-Host "Total: $($allProperties.Count) ventas (24 comunas)" -ForegroundColor Cyan
Write-Host "  Venta usado: $ventaUsado | Venta nuevo: $ventaNuevo" -ForegroundColor Cyan
