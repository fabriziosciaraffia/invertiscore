# scrape-toctoc-arriendo.ps1 - Scraper TocToc SOLO ARRIENDOS
# Correr: .\scrape-toctoc-arriendo.ps1
...
$TIPO_ARRIENDO = @{ slug = "arriendo"; filtroId = 2; filtroValue = @(3) }
...
Write-Host ""
Write-Host "FRANCO - Scraper TocToc ARRIENDOS (24 comunas, max $MAX_PAGES pags/comuna)" -ForegroundColor Red
...
Write-Host ""
Write-Host "=== ARRIENDO ===" -ForegroundColor Yellow

foreach ($comunaSlug in $COMUNAS.Keys | Sort-Object) {
    $comunaInfo = $COMUNAS[$comunaSlug]
    $props = Get-Properties -Session $session -ComunaSlug $comunaSlug -ComunaInfo $comunaInfo -Tipo $TIPO_ARRIENDO
    $allProperties += $props
...
Write-Host ""
Write-Host "Total scrapeado: $($allProperties.Count) arriendos" -ForegroundColor Cyan
...
Write-Host "Total: $($allProperties.Count) arriendos (24 comunas)" -ForegroundColor Cyan
