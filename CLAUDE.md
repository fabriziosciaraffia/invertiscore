...
- **EXCEPCIÓN APROBADA — color semántico en visualización de datos.** La regla de dos colores rige el *chrome* (texto, bordes, cards, CTAs, wordmark). **No rige dentro de un diagrama que codifica un eje ordinal**: ahí el color ES el dato y la escala de grises no lo puede expresar. Cubre las zonas por veredicto del `Dial` (COMPRAR verde `rgb(47,125,85)` · AJUSTA ámbar `rgb(169,111,27)` · BUSCAR OTRA Signal Red), los degradados del `Thermo` y cualquier primitiva futura que mapee posición→calidad. Vive en el `<style>` inyectado de `HallazgosAcordeon.tsx`. **No lo "corrijas" a Ink**: se aprobó en FASE 4.2 con shots, y sin color esas zonas dejan de ser legibles. Fuera del diagrama, la regla de dos colores sigue entera.
