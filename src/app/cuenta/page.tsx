  ...
  return (
    <div className="min-h-screen bg-th-page text-th-text">
      ...
      <nav className="sticky top-0 z-50 border-b border-th-border-strong bg-th-page">
        <div className="container mx-auto flex h-14 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/dashboard"
              className="font-body text-sm text-th-text/80 hover:text-th-text hover:bg-th-surface px-3 py-1.5 rounded-md transition-colors"
            >
              ← Dashboard
            </Link>
            <ThemeToggle />
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-heading text-2xl font-bold text-th-text">{firstName ? `${firstName}, esta es tu cuenta` : "Tu cuenta"}</h1>
          <p className="font-mono text-sm text-[#71717A] mt-1">{user.email}</p>
        </div>

        {/* ─── Sección 1: Tu Plan ─── */}
        <section className="mb-8">
          <h2 className="font-heading text-lg font-bold mb-3 text-th-text">Tu plan</h2>
          <div className="rounded-lg border border-th-border-strong bg-th-card p-6">
            {isSubscriber && (
              <>
                ...
                <p className="mt-3 font-body text-sm text-th-text">Análisis Pro ilimitados</p>
                {subEnd && (
            ...
            {isCancelled && (
              <>
                ...
                <p className="mt-3 font-body text-sm text-th-text">Tu suscripción fue cancelada</p>
                {subEnd && (
                  <p className="mt-1 font-body text-xs text-[#71717A]">
            ...
            {hasCredits && (
              <>
                ...
                <p className="mt-3 font-body text-sm text-th-text">
        ...
        <section className="mb-8">
          <h2 className="font-heading text-lg font-bold mb-3 text-th-text">Historial de compras</h2>
                  ...
                  {payments.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-th-border-strong last:border-b-0">
                        <td className="font-mono text-xs text-th-text py-2 pr-4">{fmtDate(p.created_at)}</td>
                        <td className="font-body text-xs text-th-text py-2 pr-4">{productLabel}</td>
                        <td className="font-mono text-xs text-th-text py-2 pr-4">{isCredit ? "Crédito" : fmtCLP(p.amount)}</td>
                        <td className="py-2">
        ...
        <section className="mt-16 border-t border-th-border-strong pt-8">
          <DeleteAccountButton />
        </section>
      </div>
    </div>
