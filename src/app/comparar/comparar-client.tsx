          ...
          {labels.map((label, i) => {
            ...
            return (
              <text
                ...
                className="fill-[#FAFAF8]/50 text-[10px] sm:text-[11px]"
              >
                {label}
              </text>
            );
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-4">
        {analisis.map((a, i) => (
          <div key={a.id} className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#FAFAF8]/50">{a.nombre}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#0F0F0F]">
      ...
      <div className="mx-auto max-w-5xl px-5 py-8 sm:px-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <Link href="/dashboard" className="mb-3 inline-flex items-center gap-1.5 text-sm text-[#FAFAF8]/50 transition-colors hover:text-[#FAFAF8]">
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] bg-[#151515] px-3 py-1.5 text-sm font-medium text-[#FAFAF8]/50 transition-colors hover:bg-[#1A1A1A] hover:text-[#FAFAF8]"
          >
        ...
        <div className="mt-8 hidden md:block">
          ...
          <div className="mb-4 grid gap-4" style={{ gridTemplateColumns: `180px repeat(${colCount}, 1fr)` }}>
            ...
            {analisis.map((a, i) => {
              ...
              return (
                <div key={a.id} className="rounded-xl border bg-[#151515] p-4 text-center" style={{ borderColor: CHART_COLORS[i], borderWidth: "2px" }}>
                  ...
                  <div className="text-xs text-[#FAFAF8]/50">{a.comuna}</div>
                </div>
              );
            })}
          </div>
          ...
          {sections.map((section) => (
            <div key={section.section} className="mb-2">
              ...
              >
                <span className="text-xs font-semibold uppercase tracking-wider text-[#FAFAF8]/50">{section.section}</span>
              </div>
              {section.rows.map((row, ri) => (
                <div
                  key={row.label}
                  className={`grid items-center gap-4 border-b border-white/[0.04] px-4 py-2.5 ${ri % 2 === 0 ? "bg-[#151515]" : "bg-[#0F0F0F]"}`}
                  style={{ gridTemplateColumns: `180px repeat(${colCount}, 1fr)` }}
                >
                  <span className="text-sm text-[#FAFAF8]/50">{row.label}</span>
        ...
        <div className="mt-6 space-y-6 md:hidden">
          {analisis.map((a, ai) => {
            const color = getScoreColor(a.score);
            return (
              <div key={a.id} className="rounded-xl border-2 bg-[#151515] p-4" style={{ borderColor: CHART_COLORS[ai] }}>
                {/* Card header */}
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#FAFAF8]/50">{a.comuna} · {getScoreLabel(a.score)}</div>
                  </div>
                </div>
                {/* Metrics */}
                {sections.map((section) => (
                  <div key={section.section} className="mt-4">
                    <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-[#FAFAF8]/50">{section.section}</div>
                    <div className="space-y-1">
                      {section.rows.map((row) => (
                        <div key={row.label} className="flex items-center justify-between py-1">
                          <span className="text-xs text-[#FAFAF8]/50">{row.label}</span>
        ...
        {/* Verdict */}
        <div className="mt-10 rounded-2xl border-2 border-white/[0.08] bg-[#151515] p-6 sm:p-8">
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:gap-6">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-relaxed text-[#FAFAF8]/50">{verdict.explanation}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          <Link href="/dashboard">
            <Button variant="outline" className="gap-2 rounded-xl border-white/[0.08] text-[#FAFAF8]/50 hover:text-[#FAFAF8]">
              <ArrowLeft className="h-4 w-4" /> Volver al Dashboard
            </Button>
          </Link>
        </div>
