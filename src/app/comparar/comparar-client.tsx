...
function RadarChart({ analisis }: { analisis: Analisis[] }) {
  ...
  return (
    <div className="mt-10">
      <h3 className="text-center font-heading text-xl font-bold tracking-tight text-[var(--franco-text)] sm:text-2xl">Radar de dimensiones</h3>
          ...
          {labels.map((label, i) => {
            ...
            return (
              <text
                ...
                className="fill-[var(--franco-text-secondary)] text-[10px] sm:text-[11px]"
              >
                {label}
              </text>
            );
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-4">
        {analisis.map((a, i) => (
          <div key={a.id} className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--franco-text-secondary)]">{a.nombre}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[var(--franco-bg)]">
      ...
      <div className="mx-auto max-w-5xl px-5 py-8 sm:px-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <Link href="/dashboard" className="mb-3 inline-flex items-center gap-1.5 text-sm text-[var(--franco-text-secondary)] transition-colors hover:text-[var(--franco-text)]">
              <ArrowLeft className="h-4 w-4" /> Volver al Dashboard
            </Link>
            <h1 className="font-heading text-2xl font-bold tracking-tight text-[var(--franco-text)] sm:text-3xl">
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-[var(--franco-border)] bg-[var(--franco-card)] px-3 py-1.5 text-sm font-medium text-[var(--franco-text-secondary)] transition-colors hover:bg-[var(--franco-card)] hover:text-[var(--franco-text)]"
          >
        ...
        <div className="mt-8 hidden md:block">
          ...
          <div className="mb-4 grid gap-4" style={{ gridTemplateColumns: `180px repeat(${colCount}, 1fr)` }}>
            ...
            {analisis.map((a, i) => {
              ...
              return (
                <div key={a.id} className="rounded-xl border bg-[var(--franco-card)] p-4 text-center" style={{ borderColor: CHART_COLORS[i], borderWidth: "2px" }}>
                  ...
                  <div className="mt-2 text-sm font-semibold text-[var(--franco-text)]">{a.nombre}</div>
                  <div className="text-xs text-[var(--franco-text-secondary)]">{a.comuna}</div>
                </div>
              );
            })}
          </div>
          ...
          {sections.map((section) => (
            <div key={section.section} className="mb-2">
              <div
                className="grid items-center gap-4 rounded-t-lg bg-[var(--franco-card)] px-4 py-2"
                style={{ gridTemplateColumns: `180px repeat(${colCount}, 1fr)` }}
              >
                <span className="text-xs font-semibold uppercase tracking-wider text-[var(--franco-text-secondary)]">{section.section}</span>
              </div>
              {section.rows.map((row, ri) => (
                <div
                  key={row.label}
                  className={`grid items-center gap-4 border-b border-[var(--franco-border)] px-4 py-2.5 ${ri % 2 === 0 ? "bg-[var(--franco-card)]" : "bg-[var(--franco-bg)]"}`}
                  style={{ gridTemplateColumns: `180px repeat(${colCount}, 1fr)` }}
                >
                  <span className="text-sm text-[var(--franco-text-secondary)]">{row.label}</span>
        ...
        <div className="mt-6 space-y-6 md:hidden">
          {analisis.map((a, ai) => {
            const color = getScoreColor(a.score);
            return (
              <div key={a.id} className="rounded-xl border-2 bg-[var(--franco-card)] p-4" style={{ borderColor: CHART_COLORS[ai] }}>
                {/* Card header */}
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-[var(--franco-text)]">{a.nombre}</div>
                    <div className="text-xs text-[var(--franco-text-secondary)]">{a.comuna} · {getScoreLabel(a.score)}</div>
                  </div>
                </div>
                {/* Metrics */}
                {sections.map((section) => (
                  <div key={section.section} className="mt-4">
                    <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--franco-text-secondary)]">{section.section}</div>
                    <div className="space-y-1">
                      {section.rows.map((row) => (
                        <div key={row.label} className="flex items-center justify-between py-1">
                          <span className="text-xs text-[var(--franco-text-secondary)]">{row.label}</span>
        ...
        {/* Verdict */}
        <div className="mt-10 rounded-2xl border-2 border-[var(--franco-border)] bg-[var(--franco-card)] p-6 sm:p-8">
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:gap-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-[var(--franco-text)]">
              ...
              <p className="mt-2 text-sm leading-relaxed text-[var(--franco-text-secondary)]">{verdict.explanation}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          <Link href="/dashboard">
            <Button variant="outline" className="gap-2 rounded-xl border-[var(--franco-border)] text-[var(--franco-text-secondary)] hover:text-[var(--franco-text)]">
              <ArrowLeft className="h-4 w-4" /> Volver al Dashboard
            </Button>
          </Link>
        </div>
